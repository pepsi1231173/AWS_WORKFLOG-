...
async function typeCommand(send, command) {
  ...
  await send("Input.dispatchMouseEvent", { type: "mousePressed", x: 90, y: 610, button: "left", clickCount: 1 }).catch(() => {});
  await send("Input.dispatchMouseEvent", { type: "mouseReleased", x: 90, y: 610, button: "left", clickCount: 1 }).catch(() => {});
  await wait(500);
...
async function main() {
  ...
  await wait(18000);
  await send("Runtime.evaluate", {
    expression: "document.querySelector('[aria-label=\"Close welcome modal\"]')?.click();",
  }).catch(() => {});
  ...
  const labScript = fs.readFileSync("D:/RoughLife GAME/RoughLife/aws-lab-000007/lab7-budgets-create.sh", "utf8");
  const script = `cat > lab7-budgets.sh <<'EOF'\n${labScript}\nEOF
chmod +x lab7-budgets.sh
./lab7-budgets.sh 2>&1 | tee lab7-budgets-create.log\n`;

  await typeCommand(send, script);
  let text = "";
  for (let i = 0; i < 8; i++) {
    await wait(15000);
    text = await bodyText(send).catch(() => "");
    if (text.includes("LAB7_CREATE_DONE LAB7-0704")) break;
  }
  fs.mkdirSync(OUT_DIR, { recursive: true });
  fs.writeFileSync(`${OUT_DIR}/lab7-create-cloudshell-text.txt`, text);
  await screenshot(send, "01-cloudshell-created-budgets.png");
