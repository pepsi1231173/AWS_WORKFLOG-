...
async function main() {
  ...
  await send("Input.dispatchMouseEvent", { type: "mousePressed", x: 1465, y: 640, button: "left", clickCount: 1 }).catch(() => {});
  await send("Input.dispatchMouseEvent", { type: "mouseReleased", x: 1465, y: 640, button: "left", clickCount: 1 }).catch(() => {});
  ws.close();
}
main().catch((err) => {
