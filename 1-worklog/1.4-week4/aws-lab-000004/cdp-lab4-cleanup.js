...
async function main() {
  ...
  const labScript = fs.readFileSync(`${ROOT}/cleanup-lab4.sh`, "utf8");
  const command = `cat > cleanup-lab4.sh <<'EOF'\n${labScript}\nEOF\nchmod +x cleanup-lab4.sh\n./cleanup-lab4.sh 2>&1 | tee cleanup-lab4.log\n`;
  ...
  for (let i = 0; i < 30; i++) {
    ...
    fs.writeFileSync(`${OUT_DIR}/14-cloudshell-cleanup-text.txt`, text);
    if (text.includes("LAB4_CLEANUP_DONE LAB4-0705")) break;
    console.log(`waiting cleanup ${i + 1}/70`);
  }

  await screenshot(send, "14-cloudshell-cleanup-done.png");
